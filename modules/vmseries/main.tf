...
resource "azurerm_network_security_group" "mgmt" {
  ...
  name                = "${var.hubprefixpalo}-mgmt-nsg"
...
resource "azurerm_network_security_group" "data" {
  ...
  name                = "${var.hubprefixpalo}-data-nsg"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_public_ip" "nic0" {
  ...
  #name                = "${var.name}${count.index + 1}-nic0-pip-0${count.index + 1}"
  name                = "${var.name}${count.index + 1}-nic0-pip-01"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_public_ip" "nic1" {
  ...
  #name                = "${var.name}${count.index + 1}-nic1-pip-0${count.index + 1}"
  name                = "${var.name}${count.index + 1}-nic1-pip-01"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = var.public_ip_address_allocation
  sku                 = var.sku
...
resource "azurerm_public_ip" "nic2" {
  ...
  #name                = "${var.name}${count.index + 1}-nic2-pip-0${count.index + 1}"
  name                = "${var.name}${count.index + 1}-nic2-pip-01"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = var.public_ip_address_allocation
  sku                 = var.sku
